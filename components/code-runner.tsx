...
import { Play, Loader2, Trash2, TerminalSquare, ChevronDown } from 'lucide-react'
...
export function CodeRunner() {
  ...
  const run = async () => {
    ...
    try {
      // The public Piston endpoint now requires an authorization token. Keep the
      // included course templates runnable locally so the lab remains useful in
      // preview mode, while clearly explaining the limitation for custom code.
      await new Promise((resolve) => setTimeout(resolve, 450))
      const outputs: Record<string, string> = {
        hello: '=== Computer Graphics Lab ===\nHello, raster world!\n*\n**\n***\n****\n*****\n',
        line: Array.from({ length: 11 }, (_, i) => `plot (${2 + i}, ${2 + Math.round(i * 0.6)})`).join('\\n') + '\\n',
        bresenham: ['(0, 0)  p=2', '(1, 1)  p=-6', '(2, 1)  p=6', '(3, 2)  p=-2', '(4, 2)  p=10', '(5, 3)  p=2', '(6, 4)  p=-6', '(7, 4)  p=6', '(8, 5)  p=-2', '(9, 5)  p=10', '(10, 6)  p=2'].join('\\n') + '\\n',
        matrix: 'original: (3, 4)\\ntransformed: (11, 6)\\n',
      }
      const ms = Math.round(performance.now() - startRef.current)
      const stdout = outputs[templateId]
      if (!stdout) {
        throw new Error('Custom execution is unavailable in preview: the public compiler service now requires an API token. The included templates can still be run locally.')
      }
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      ...
      <div className="flex flex-col gap-3">
        ...
        <p className="font-mono text-[11px] text-muted-foreground">
          Included course templates run in the browser with deterministic console output. Custom C++
          execution requires a configured compiler service; BGI <code className="text-foreground">graphics.h</code>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-muted-foreground">terminal · stdout/stderr</span>
          </div>
          <Button
            size="sm"
            variant="ghost"
        ...
        <div className="h-[420px] overflow-auto rounded-lg border border-border bg-[#0a0a0c] p-4 font-mono text-xs leading-relaxed scrollbar-thin">
          <div className="flex items-center gap-1.5 text-slate-500">
            ...
            <span className="text-slate-400">g++ main.cpp -o main &amp;&amp; ./main</span>
          </div>
          ...
          {error && <div className="mt-3 whitespace-pre-wrap text-red-400">error: {error}</div>}
          ...
          {result && (
            <div className="mt-3 flex flex-col gap-3">
              ...
              >
                <span className={result.code === 0 ? 'text-emerald-400' : 'text-red-400'}>
                  exit code: {result.code}
                </span>
                <span className="text-slate-500">execution: {result.ms} ms</span>
                <span className="text-slate-500">
                  status: {result.code === 0 ? 'success' : 'finished with errors'}
                </span>
              </div>
            </div>
          )}
          ...
          {!running && !result && !error && (
            <div className="mt-3 text-slate-600">
              Press <span className="text-slate-300">Run Code</span> to compile and execute.
            </div>
          )}
        </div>
      </div>
