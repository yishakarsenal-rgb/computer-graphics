...
const BGI: string[] = [
  ...
  '#545454', // 8 dark gray
  '#5454fc', // 9 light blue
  '#54fc54', // 10 light green
  '#54fcfc', // 11 light cyan
  '#fc5454', // 12 light red
  '#fc54fc', // 13 light magenta
  '#fcfc54', // 14 yellow
  '#ffffff', // 15 white
]
    ...
    const step = () => {
      ...
      switch (sim) {
        ...
        case 'ellipse': {
          ...
          ctx.beginPath()
          ctx.moveTo(500, 100)
          ctx.ellipse(500, 100, 80, 50, 0, -(80 * Math.PI) / 180, -(10 * Math.PI) / 180, true)
          ctx.closePath()
          ctx.fill()
          ctx.strokeStyle = BGI[15]
          ctx.stroke()
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button size="sm" variant="outline" onClick={reset} aria-label="Reset animation">
            <RotateCcw />
            Reset
          </Button>
        </div>
      ...
      {isInteractive && (
        <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            <span className="font-mono text-foreground">arrow keys</span> to steer the pen and draw
            connected segments, just like the C++ program.
          </p>
        </div>
      )}
    </div>
